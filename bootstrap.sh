...
function init_validator() {
     ...
     geth --datadir ${workspace}/storage/${node_id} account new   --password /home/developer/node-config/genesis/pass.txt > ${workspace}/storage/${node_id}Info
...
function generate_genesis() {
     ...
     sed  "s/{{INIT_HOLDER_ADDRESSES}}/${INIT_HOLDER_ADDRESSES}/g" ${workspace}/genesis/init_holders.template | sed  "s/{{INIT_HOLDER_BALANCE}}/500000000000000000000/g" > ${workspace}/genesis/init_holders.js
     node generate-validator.js
     chainIDHex=$(printf '%04x\n' "1306")
     ./create-genesis config.json > genesis.json #create genesis.json file 
     # node generate-genesis.js --chainid "1306" --bscChainId ${chainIDHex}
}
...
function init_genesis_data() {
     ...
     node_type=$1
     node_id=$2
     geth --datadir=${workspace}/storage/${node_id} --genesis=${workspace}/genesis/genesis.json init ${workspace}/genesis/genesis.json
     cp ${workspace}/config/config-${node_type}.toml  ${workspace}/storage/${node_id}/config.toml
     sed -i -e "s/{{NetworkId}}/${FXCE_CHAIN_ID}/g" ${workspace}/storage/${node_id}/config.toml
     if [ "${node_id}" == "fxce-rpc" ]; then
...
# for((i=1;i <= 2;i++)); do
#      init_validator "fxce-validator${i}"
# done
...
init_genesis_data fxce-rpc fxce-rpc
